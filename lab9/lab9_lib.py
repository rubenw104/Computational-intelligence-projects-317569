...
class AbstractProblem:
    ...
    def __call__(self, genome):
        self._calls += 1                               #s is the starting point index. self.x is the step
        fitnesses = sorted((AbstractProblem.onemax(genome[s :: self.x]) for s in range(self.x)), reverse=True) #descending
